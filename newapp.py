...
if df["BMI"].values[0] < 18.5:
    st.write("You have a low BMI. Consider strength training and resistance exercises to build muscle mass.")
elif df["BMI"].values[0] < 25:
    st.write("You have a normal BMI. A mix of cardio and strength training will help maintain fitness.")
else:
    st.write("Your BMI is high. Focus on cardio workouts such as jogging, cycling, or swimming to aid weight loss.")

if df["Heart_Rate"].values[0] > 100:
    st.write("Your heart rate is elevated. Consider moderate-intensity exercises like brisk walking or yoga.")
else:
    st.write("Your heart rate is in a normal range. You can try high-intensity workouts like HIIT or sprinting.")

#calorie Goal 
st.sidebar.header("Calorie Goal")
calorie_goal = st.sidebar.number_input("Enter your target calories to burn (kcal):", min_value=0, value=500, step=10)
if prediction[0] > 0:
    calories_per_min = prediction[0] / df["Duration"].values[0]
    additional_time = max(0, (calorie_goal - prediction[0]) / calories_per_min)
    st.header("Exercise Duration Suggestion: ")
    if additional_time > 0:
        st.write(f"To reach {calorie_goal} kcal, you need to exercise for **{round(additional_time, 2)}** more minutes.")
    else:
        st.write("Great job! You have already met or exceeded your calorie goal!")
